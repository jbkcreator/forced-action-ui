...
import { wlResetPassword } from '../api/whiteLabelClient.js';

export default function WLResetPasswordPage() {
  ...
  async function handleSubmit(e) {
    ...
    setError('');
    try {
      await wlResetPassword(token, password);
      navigate('/wl/login?reset=1');
    } catch (err) {
    ...
    <div className="min-h-screen bg-fa-bg-base flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          <Link to="/wl/login" className="block text-center text-sm text-fa-text-muted hover:text-fa-text-primary">← Back to login</Link>
        </form>
      </div>
    </div>
  );

...
export default function WaitlistForm({ zip, countyId, waitlistType = 'sold_out', vertical: initialVertical }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err?.status === 'already_registered') {
        setMsg({ type: 'success', text: `✓ You're already on the list for ZIP ${zip}.` });
      } else {
    ...
    <section className="max-w-6xl mx-auto px-6 pb-6">
      <div className="glass-card rounded-2xl p-8 max-w-xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-[0.2em] text-yellow-400/70 mb-2">Trade</label>
            <select
          ...
          <button
            type="submit"
            disabled={submitting}
            className="btn-primary w-full bg-yellow-400 hover:bg-yellow-300 text-black font-bold py-3.5 rounded-xl disabled:opacity-50"
          >
